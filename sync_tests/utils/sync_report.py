...
def generate_sync_report(file_list: list, output_dir: str = "") -> None:
    """Process a list of JSON files, loading their content and generate sync graphs."""
    logging.info("Starting the sync report generation.")
    ...
    for file_path in file_list:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                log_data[dataset_name] = {
                    k: v for k, v in sorted(log_values_dict.items(), key=lambda item: item[0])
                }

                # Sync duration per epoch
                sync_duration_dict = ast.literal_eval(
                    str(sync_results["sync_duration_per_epoch"]))
                sync_duration_data[dataset_name] = sync_duration_dict

                # Sync time per Era
                sync_time_per_era = {}
                for era in eras:
                    sync_time_per_era[era] = sync_results[f"{era}_sync_duration_secs"]
            ...
            logging.exception(f"Unexpected error with file {file_path}: {e}")

    logging.info("Generating sync results graphs.")
...
def generate_resource_consumption_graphs(datasets: dict, output_dir: str) -> None:
    ...
    def _add_traces(fig, data_type, unit_conversion=1):
    ...
    logging.info("Successfully generated sync resource consumption graphs.")
...
def generate_duration_per_epoch_graphs(datasets: dict, output_dir: str) -> None:
    ...
    for idx, (dataset_name, data) in enumerate(datasets.items()):
        fig_duration_per_epoch.add_trace(go.Scatter(
    ...
    logging.info("Successfully generated duration per epoch graph.")
...
def generate_sync_time_per_era_graphs(datasets: dict, eras: list, output_dir: str) -> None:
    ...
    for era in eras:
        ...
        fig_time_per_era.add_trace(go.Bar(
            x=datasets_names,
            y=era_values,
            name=era,
        ))
    ...
    logging.debug("Successfully generated sync time per era graph.")


def main() -> int:
    """Main function to generate sync tests report."""
    logging.basicConfig(
    ...
    generate_sync_report(file_list=args.json_files, output_dir=args.output_dir or "")
