...
def filtered_log_fd(log_file: Path, use_rg: bool = False) -> subprocess.Popen:
    ...
    if use_rg:
        cmd = ["rg", pattern, str(log_file)]
    else:
        cmd = ["grep", "-E", pattern, str(log_file)]
...
def get_db_sync_data_from_logs(log_file: Path) -> dict[str, tp.Any]:
    ...
    Returns:
        ...
        - 'epoch_timings': Dict mapping epoch number → {start_time, end_time, duration_sec, blocks_count}
        - 'block_insertions': List of dicts with {timestamp, epoch, slot, block, era}
        - 'epoch_details': Dict mapping epoch number → aggregated stats
    """
    chunk_size = 512 * 1024  # 512 KB
    
    # Data structures for collecting metrics
    epoch_timings: dict[int, dict[str, tp.Any]] = {}  # epoch_no → {start_time, end_time, duration_sec, blocks_count}
    block_insertions: list[dict[str, tp.Any]] = []  # List of block insertion events
    epoch_details: dict[int, dict[str, tp.Any]] = {}  # epoch_no → aggregated stats
    
    # Track current epoch being processed
    current_epoch: int | None = None
    current_epoch_start_time: datetime.datetime | None = None
    current_epoch_blocks: int = 0
    last_timestamp: datetime.datetime | None = None  # Track last seen timestamp for lines without timestamps
    
    # Regex patterns
    timestamp_pattern = re.compile(r"\[(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{2}) UTC\]")
    block_insert_pattern = re.compile(
        r"Insert (Alonzo|Babbage|Conway|Byron|Shelley|Mary|Allegra) Block: epoch (\d+), slot (\d+), block (\d+)"
    )
    ...
    def _process_log_line(line: str) -> None:
        ...
        # Skip processing if we still don't have a timestamp (unless it's a pattern that doesn't need one)
        if timestamp is None:
            ...
            if took_match and current_epoch is not None and last_timestamp is not None:
                ...
                if current_epoch not in epoch_timings:
                    epoch_timings[current_epoch] = {
                        "start_time": current_epoch_start_time.isoformat() if current_epoch_start_time else None,
        ...
        if block_match:
            ...
            if epoch == current_epoch:
                ...
                epoch_details[epoch]["blocks_count"] = epoch_details[epoch].get("blocks_count", 0) + 1
        ...
        if completed_match:
            ...
            if epoch not in epoch_timings:
                ...
                if epoch_timings[epoch]["end_time"] is None:
