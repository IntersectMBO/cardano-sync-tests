...
def git_clone_iohk_repo(repo_name, repo_dir, repo_branch):
    ...
    try:
        ...
        logging.info(f"Repository {repo_name} successfully cloned to {repo_dir} and branch {repo_branch} checked out.")
        return repo
...
def git_checkout(repo, rev):
    ...
    try:
        ...
        logging.error(f"Error checking out revision {rev} in repository {repo.working_dir}: {e}")
        raise
...
def clone_repo(repo_name, repo_branch):
    ...
    try:
        ...
        logging.error(f"Error cloning repository {repo_name} to {location}: {e}")
        raise
