...
def upload_sync_results_to_aws(env):
    ...
    test_id = str(int(aws_db.get_last_identifier(test_summary_table).split("_")[-1]) + 1)
...
def print_report(db_schema, db_indexes):
    ...
    if db_schema:
        db_sync.print_color_log(
            db_sync.sh_colors.WARNING, f"Db schema issues: {db_schema}"
        )
    else:
        db_sync.print_color_log(db_sync.sh_colors.WARNING, "NO Db schema issues")
    if db_indexes:
        db_sync.print_color_log(
            db_sync.sh_colors.WARNING, f"Db indexes issues: {db_indexes}"
        )
    else:
...
def main():
    ...
    # cardano-node setup
    NODE_DIR = gitpython.clone_repo("cardano-node", node_version_from_gh_action)
    os.chdir(NODE_DIR)
    helpers.execute_command("nix build -v .#cardano-node -o cardano-node-bin")
    helpers.execute_command("nix-build -v -A cardano-cli -o cardano-cli-bin")

    print("--- Node setup")
    node.copy_node_executables(build_method="nix")
    node.get_node_config_files(env)
    db_sync.set_node_socket_path_env_var_in_cwd()
    ...
    epoch_no, block_no, slot_no = db_sync.get_db_sync_tip(env)
    end_test_time = datetime.now().strftime("%d/%m/%Y %H:%M:%S")

    print("--- Summary & Artifacts uploading")
    print(
        f"FINAL db-sync progress: {db_sync.get_db_sync_progress(env)}, epoch: {epoch_no}, block: {block_no}"
    )
    print(f"TOTAL sync time [sec]: {db_full_sync_time_in_secs}")

    # shut down services
    db_sync.manage_process(proc_name="cardano-db-sync", action="terminate")
    db_sync.manage_process(proc_name="cardano-node", action="terminate")
