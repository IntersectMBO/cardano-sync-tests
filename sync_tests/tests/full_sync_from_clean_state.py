...
def main():
    ...
    env = utils_db_sync.get_arg_value(args, "environment")
    print(f"Environment: {env}")

    node_pr = utils_db_sync.get_arg_value(args, "node_pr")
    print(f"Node PR number: {node_pr}")

    node_branch = utils_db_sync.get_arg_value(args, "node_branch")
    print(f"Node branch: {node_branch}")

    node_version_from_gh_action = utils_db_sync.get_arg_value(args, "node_version_gh_action")
    print(f"Node version: {node_version_from_gh_action}")

    db_branch = utils_db_sync.get_arg_value(args, "env")
    print(f"DB sync branch: {db_branch}")

    db_start_options = utils_db_sync.get_arg_value(args, "db_start_options")

    db_sync_version_from_gh_action = utils_db_sync.get_arg_value(args, "db_sync_version_gh_action")
