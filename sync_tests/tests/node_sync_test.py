...
def enable_cardano_node_resources_monitoring(node_config_filepath):
    ...
    helpers.print_message(type="info", message=
...
def stop_node(platform_system):
    for proc in process_iter():
        if 'cardano-node' in proc.name():
            helpers.print_message(type="info_warn", message=f"Killing the `cardano-node` process - {proc}")
    ...
    for proc in process_iter():
        if 'cardano-node' in proc.name():
            helpers.print_message(type="error", message=f" !!! ERROR: `cardano-node` process is still active - {proc}")
...
def get_cli_executable_path_built_with_cabal():
    for f in get_cabal_build_files():
        if "\\x\\cardano-cli\\build\\" in f and 'cardano-cli-tmp' not in f and 'autogen' not in f:
            helpers.print_message(type="info", message=f"Found node-cli executable: {f}")
            global CLI
            CLI = f
            return f
...
def get_node_files(node_rev, repository=None, build_tool='nix'):
    ...
    if build_tool == 'nix':
        ...
        node.copy_node_executables("nix")
        ...
        node.copy_node_executables("cabal")
...
def main():
    ...
    helpers.print_message(type="info", message=f"Get the cardano-node and cardano-cli files using - {node_build_mode}")
    ...
    if 'windows' not in platform_system.lower():
        ...
        helpers.print_message(type="error", message=
    ...
    cli_version1, cli_git_rev1 = node.get_cli_version()
    ...
    helpers.print_message(type="ok", message="===================================================================================")
    helpers.print_message(type="ok", message=f"================== Start node sync test using node_rev1: {node_rev1} =============")
    helpers.print_message(type="ok", message="===================================================================================")
    print('')
    ...
    sync_time_seconds1, last_slot_no1, latest_chunk_no1, era_details_dict1, epoch_details_dict1 = node.wait_for_node_to_sync(
        env)
    ...
    if tag_no2 != 'None':
        ...
        print('')
        helpers.print_message(type="ok", message="==============================================================================")
        helpers.print_message(type="ok", message=f"================= Start sync using node_rev2: {node_rev2} ===================")
        helpers.print_message(type="ok", message="==============================================================================")
        ...
        helpers.print_message(type="warn", message='node version')
        cli_version2, cli_git_rev2 = node.get_cli_version()
        ...
        helpers.print_message(type="info", message=f" - waiting for the node to sync - using node_rev2: {node_rev2}")
        sync_time_seconds2, last_slot_no2, latest_chunk_no2, era_details_dict2, epoch_details_dict2 = node.wait_for_node_to_sync(
            env)
