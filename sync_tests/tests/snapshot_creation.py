...
TEST_RESULTS = f"snapshot_creation_{configuration.ENVIRONMENT}_test_results.json"
...
def upload_snapshot_creation_results_to_aws(env: str) -> None:
    ...
    last_identifier = aws_db.get_last_identifier(db_snapshot_creation_test_summary_table)
...
def main() -> int:
    ...
    db_sync_pr = helpers.get_arg_value(args=args, key="db_sync_pr", default="")
    ...
    stage_2_cmd = db_sync.create_db_sync_snapshot_stage_1(env)
    print(f"Stage 2 command: {stage_2_cmd}")
    stage_2_result = db_sync.create_db_sync_snapshot_stage_2(stage_2_cmd, env)
    ...
    if env != "mainnet":
        db_sync.upload_artifact(snapshot_file)
