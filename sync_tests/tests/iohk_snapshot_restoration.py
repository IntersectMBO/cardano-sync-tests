...
def main():
    ...
    # cardano-node setup
    NODE_DIR = gitpython.clone_repo("cardano-node", node_branch)
    os.chdir(NODE_DIR)
    ...
    db_sync.download_and_extract_node_snapshot(env)
    node.start_node(env)
    ...
    snapshot_epoch_no, snapshot_block_no, snapshot_slot_no = db_sync.get_db_sync_tip(env)
    print(
